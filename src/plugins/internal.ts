...
//@ts-ignore
          for (const cat of (window.Surakku.plugins as Plugin[][])) {
            for (const plugin of cat) { 
              ...
              pluginDiv.className = "c-button-unstyled p-prefs_modal__channel_overrides_row__details"
              ...
              switchEl.checked = localStorage.getItem("plugins__" + plugin.name + "_enabled") == "true"
              switchEl.addEventListener("change", (e) => {
                localStorage.setItem("plugins__" + plugin.name + "_enabled", switchEl.checked.toString())
              })
