export function createPopup(el:HTMLElement) {
       ...
       form.style.backgroundColor = "rgba(var(--sk_primary_background,255,255,255),1)";
...
if(!el.style.color)    el.style.color = "var(--dt_color-content-pry) !important"
Array.from(el.querySelectorAll('*')).forEach(d=>{
    //@ts-ignore
    if(!d.style.color)    d.style.color = "var(--dt_color-content-pry) !important"
    if(d.tagName == "INPUT") {
        //@ts-ignore
