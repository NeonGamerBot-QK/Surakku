...
export function watchSideBarChannels() {
    setInterval(() => {
        const sidebarChannels: RawChannel[] = Array.from(document.querySelectorAll("[data-qa-channel-sidebar-channel-id]")).map(rc => {
            ...
            return {
                ...
                userCount: parseInt(rc.getAttribute("data-qa-channel-sidebar-channel-user-count")!),
                sectionChannelIndex:parseInt( rc.getAttribute("data-qa-channel-sidebar-channel-section-channel-index")!),
                isSelected: Boolean(rc.getAttribute("data-qa-channel-sidebar-channel-is-selected")!),
                isMuted: Boolean(rc.getAttribute("data-qa-channel-sidebar-channel-is-muted")!),
                sectionId: rc.getAttribute("data-qa-channel-sidebar-channel-section-id")!,
                isStarred: Boolean(rc.getAttribute("data-qa-channel-sidebar-channel-is-starred")!),
        ...
        for (const channel of channelsToModify) {
            ...
            if (sidebarChannel && channel.filter ? channel.filter(sidebarChannel) : true) {
                if ("name" in channel) {
                    const el = document.querySelector(`[data-qa-channel-sidebar-channel-id="${channel.id}"]`)
